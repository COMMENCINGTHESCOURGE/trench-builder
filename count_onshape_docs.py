...
try:
    ...
    total_docs = 0
    next_url = 'https://cad.onshape.com/api/v10/documents?limit=20'
    
    while next_url:
        docs = api(next_url)
        if 'items' in docs:
            total_docs += len(docs['items'])
        
        # Check for pagination
        next_url = None
        if 'next' in docs and docs['next']:
            next_url = docs['next']
            
    print(f"\nTOTAL ONSHAPE PROJECTS/DOCUMENTS: {total_docs}")
